...
>(
	...
	const polling = async () => {
		...
		loading.value = false
		option.onFinish && option.onFinish(data.value)
		useTimeoutPoll(polling, option.pollingInterval as number)
	}
	// 自动调用
	option.autoRun && run()
	// 是否轮询
	...
	const runComputed = computed(() => {
		...
		if (option.debounce)
			return {
				run: useDebounceFn(run, option.throttleInterval) as () => Promise<void>,
				runParams: useDebounceFn(runParams, option.throttleInterval) as (
					p: Params
				) => Promise<void>
			}
		// 判断是否开启节流
		if (option.throttle)
			return {
				run: useThrottleFn(run, option.throttleInterval) as () => Promise<void>,
				runParams: useThrottleFn(runParams, option.throttleInterval) as (
					p: Params
				) => Promise<void>
			}
